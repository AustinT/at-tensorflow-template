...
class Logger:
    def __init__(self, sess, config):
        ...
        self.train_summary_writer = tf.summary.FileWriter(os.path.join(summary_dir, "train"),
                                                          self.sess.graph)
        self.test_summary_writer = tf.summary.FileWriter(os.path.join(summary_dir, "test"))
                    ...
                    if tag not in self.summary_ops:
                        if len(value.shape) <= 1:
                            ...
                            self.summary_placeholders[tag] = \
                                tf.placeholder('float32', [None] + list(value.shape[1:]), name=tag)
